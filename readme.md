...
- **帳號**：`root`
- **密碼**：`7777`
- **允許 AI 操作**：後續 AI 可直接使用 `docker exec` 進入並執行 SQL（已授權）

常用指令：

```bash
docker exec lineage-mysql mysql -uroot -p7777 -e "SHOW DATABASES;"
docker exec lineage-mysql mysql -uroot -p7777 -e "USE l1jdb; SHOW TABLES;"
```

### 重要：nameid 查詢（$ 必須轉義）

`etc_items` 的 `unidentified_name_id` / `identified_name_id` 都是像 `$27` 這種字串。  
在 shell 中 `$` 會被展開成環境變數，**不轉義就會變成空字串**，導致查詢失敗。

正確範例（請務必照此格式）：

```bash
docker exec lineage-mysql mysql -uroot -p7777 -e "USE jpdbbackup; SELECT id,name,unidentified_name_id,identified_name_id,use_type FROM etc_items WHERE unidentified_name_id = '\$27';"
docker exec lineage-mysql mysql -uroot -p7777 -e "USE jpdbbackup; SELECT id,name,unidentified_name_id,identified_name_id,use_type FROM etc_items WHERE identified_name_id = '\$260';"
```

錯誤示例（$ 未轉義，會查不到）：

```bash
docker exec lineage-mysql mysql -uroot -p7777 -e "USE jpdbbackup; SELECT * FROM etc_items WHERE unidentified_name_id = '$27';"
```
...
- **l1jdb**  
  - JP 服務器實際連線的 **主資料庫**（目前線上使用）
  - 結構為 JP schema，內容已改為 182 時代資料
- **jpdbbackup**  
  - JP 原始資料庫備份  
  - 用途：恢復/比對/補回 JP 原始資料
- **182**  
  - 182 時代原始資料庫（Lineage 182）
  - 用途：資料來源（NPC/物品/魔法等）

### 操作規則（避免誤刪）

1. **先 SELECT 再 DELETE**：先統計再執行刪除  
2. **只動 l1jdb**：除非是讀取來源（`jpdbbackup` / `182`）  
3. **刪除前以 map_ids / maps.csv 為標準**  
4. **需要批次清理時，可先備份 SQL 或表**

---

### 物品表合併（items）

- `etc_items` / `weapons` / `armors` 合併為單一 `items` 表（以 182 `items` 為基礎）
- `item_type` 已移除（改由 `use_type` + 既有欄位推導）
- `unidentified_name_id` / `identified_name_id` 統一為 `nameid`
- `max_charge_count` 改為 `charge_count`

**TODO 驗證重點**：
1. `type=accessory` 對應的正確裝備欄位（目前暫映射為 ring）
2. `light` / `spellicon` 等類別在移除 `item_type` 後的推導規則
...
- 料理系統（Cooking）已**徹底刪除**  
  - 刪除 `CookingRecipeTable` / `L1Cooking` / `L1CookingRecipe` / `L1CookingIngredient` / `L1DishBuff`  
  - 移除 `C_UseItem` 料理道具處理分支  
  - 移除 `L1SkillId` 料理技能常數（`COOKING_*` / `COOKING_BEGIN` / `COOKING_END`）  
  - 移除 `L1SkillUse` / `C_LoginToServer` / `CharBuffTable` 的料理 Buff 處理  
  - 移除 `L1Attack` / `L1Magic` / `L1Character` / `CalcExp` 的料理加成與減傷  
  - 移除 DB 表：`cooking_recipes`, `cooking_ingredients`
  - 刪除 DB schema/csv：`db/schema/mysql/cooking_*.sql`, `db/csv/*/cooking_*.csv`
