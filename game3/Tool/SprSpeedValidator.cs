...
{
    /// <summary>
    /// 驗證 list.spr 與 SprDataTable.cs 的速度數據一致性
    /// 從 list.spr 提取角色的 walk、攻擊速度，轉換為 ms，並與 SprDataTable.cs 對比
    /// </summary>
    public partial class SprSpeedValidator : Node
    {
        // 角色 GfxId 映射
        ...
        {
            GD.Print("[SprSpeedValidator] 開始驗證速度數據...");
            
            string listSprPath = "res://Assets/list.spr";
            if (!Godot.FileAccess.FileExists(listSprPath))
            {
                GD.PrintErr($"[SprSpeedValidator] 找不到文件: {listSprPath}");
                return;
            }
            ...
            // 2. 從 SprDataTable.cs 讀取數據（需要手動解析或從代碼中提取）
            var sprDataTableData = LoadSprDataTableData();
            
            // 3. 生成對比報告
            GenerateComparisonReport(listSprData, sprDataTableData);
            
            GD.Print("[SprSpeedValidator] 驗證完成！報告已保存到 Tool/spr_speed_comparison.txt");
        }

        private Dictionary<int, CharacterSpeedData> ParseListSpr(string filePath)
        {
            ...
            if (file == null)
            {
                GD.PrintErr($"[SprSpeedValidator] 無法打開文件: {filePath}");
                return result;
            }
        ...
        private Dictionary<int, Dictionary<int, int>> LoadSprDataTableData()
        {
            ...
            Dictionary<int, Dictionary<int, int>> sprDataTableData)
        {
            ...
            foreach (var gfxId in CharacterNames.Keys.OrderBy(x => x))
            {
                ...
                foreach (var categoryGroup in byCategory)
                {
                    ...
                    foreach (var action in categoryGroup.OrderBy(a => a.ActionId))
                    {
                        ...
                        report.AppendLine($"    list.spr:     {action.TotalMs}ms (幀: {string.Join("+", action.FrameDurations)}ms)");
                        
                        if (hasSprDataTable)
                        {
                            int diff = Math.Abs(action.TotalMs - sprDataTableValue);
            ...
            // 保存報告
            string reportPath = "res://Tool/spr_speed_comparison.txt";
            using var file = Godot.FileAccess.Open(reportPath, Godot.FileAccess.ModeFlags.Write);
            if (file != null)
            {
                file.StoreString(report.ToString());
                file.Close();
            }
            else
            {
                GD.PrintErr($"[SprSpeedValidator] 無法寫入報告文件: {reportPath}");
            }
        ...
        private string GetCategoryName(ActionCategory category)
        {
