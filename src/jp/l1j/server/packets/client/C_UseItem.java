...
import jp.l1j.server.model.L1Clan;
import jp.l1j.server.model.L1DragonSlayer;
import jp.l1j.server.model.L1Furniture;
import jp.l1j.server.model.L1HouseLocation;
import jp.l1j.server.model.L1ItemDelay;
import jp.l1j.server.model.L1Location;
import jp.l1j.server.model.L1Object;
...
public class C_UseItem extends ClientBasePacket {
	...
	public C_UseItem(byte abyte0[], ClientThread client) throws Exception {
		...
		int use_type = item.getItem().getUseType();
		if (use_type == 16 || use_type == 70) { // 変身スクロール(sosc) / 変身アイテム(poly): クライアント側HTML選択 → polyName
			s = readS();
		} else if (use_type == 7 || use_type == 14 || use_type == 26
				|| use_type == 27) {
			// 確認スクロール(identify)、choice、武器強化スクロール(dai)、
		...
		if (item.getItem().getType2() == 0) { // 種別：その他のアイテム
			...
			if (itemId == 40003) { // ランタン オイル
				...
				if (!applyEtcItemPotionEffect(pc, item)) {
					pc.sendPackets(new S_ServerMessage(74, item.getLogName()));
				}
			} else if (itemId == 40070) { // 進化の実
				pc.sendPackets(new S_ServerMessage(76, item.getLogName()));
				...
				// 不確かなマリスエレメント武器 (DB-driven: 無 handler 時警告)
				_log.warning("C_UseItem: item_id=" + itemId + " (UnknownMaliceWeapon) has no DB handler.");
				pc.sendPackets(new S_ServerMessage(74, item.getLogName()));
			} else if (itemId == 50501) { // ドラゴンキー
				if (!L1CastleLocation.checkInAllWarArea(pc.getLocation())) {
					pc.sendPackets(new S_DragonGate(pc,
				...
				// くるった時計 (DB-driven: 無 handler 時警告)
				_log.warning("C_UseItem: item_id=" + itemId + " (SpeedUpClock) has no DB handler.");
				pc.sendPackets(new S_ServerMessage(74, item.getLogName()));
				...
				pc.sendPackets(new S_ServerMessage(452, item.getLogName()));
				// %0が選択されました。
			} else if (item.getItem().getType() == 20) { // スペルアイコン (DB-driven: 無 handler 時警告)
				_log.warning("C_UseItem: item_id=" + itemId + " (SpellIcon type=20) has no DB handler.");
				pc.sendPackets(new S_ServerMessage(74, item.getLogName()));
			} else if (item.getItem().getType() == 18 // スペルスクロール
					|| item.getItem().getType() == 19 // エボニーワンド
					) {
				_log.warning("C_UseItem: item_id=" + itemId + " (SpellItem type=18/19) has no DB handler.");
				pc.sendPackets(new S_ServerMessage(74, item.getLogName()));
			} else if (item.getItem().getType() == 2) { // light系アイテム
				useLight(pc, item);
			} else if (item.getItem().getType() == 7) { // 食べ物(food)
				eatFood(pc, item);
				pc.getInventory().consumeItem(itemId, 1);
			} else if (item.getItem().getType() == 16) { // treasure_box (DB-driven: 無 drop 表時警告)
				_log.warning("C_UseItem: item_id=" + itemId + " (treasure_box) has no DB handler. Add drop table.");
				pc.sendPackets(new S_ServerMessage(74, item.getLogName()));
			} else if (item.getItem().getType() == 5) { // 花火(firecracker) (DB-driven: 無 handler 時警告)
				_log.warning("C_UseItem: item_id=" + itemId + " (firecracker) has no DB handler.");
				pc.sendPackets(new S_ServerMessage(74, item.getLogName()));
			} else if (item.getItem().getType() == 21) { // 蒸発保護スクロール (DB-driven)
				L1ItemInstance target = pc.getInventory().getItem(objid);
				if (target != null) {
					target.setProtected(true);
					target.setProtectItemId(item.getItemId());
					pc.getInventory().updateItem(target, L1PcInventory.COL_ENCHANTLVL);
					pc.getInventory().removeItem(item, 1);
				} else {
					_log.warning("C_UseItem: protect scroll target objid=" + objid + " not found.");
					pc.sendPackets(new S_ServerMessage(74, item.getLogName()));
				}
			} else if (item.getItem().getType() == 22) { // ユニーク強化スクロール (DB-driven: 無 handler 時警告)
				_log.warning("C_UseItem: item_id=" + item.getItemId() + " (UniqueEnchantScroll) has no DB handler.");
				pc.sendPackets(new S_ServerMessage(74, item.getLogName()));
			} else if (use_type == 6 || use_type == 29) {
				// テレポートスクロール(ntele)、祝福されたテレポートスクロール(btele)
				if(useTeleportScroll(pc, item, objid)) {
			...
			} else if (use_type == 14) { // choice (DB-driven: 無 handler 時警告)
				_log.warning("C_UseItem: item_id=" + itemId + " (MaterialChoice use_type=14) has no DB handler.");
				pc.sendPackets(new S_ServerMessage(74, item.getLogName()));
			} else if (use_type == 16) { // 変身スクロール(sosc): readS() → PolyTable lookup → doPoly
				if (s != null && !s.isEmpty()) {
					L1PolyMorph.handleCommands(pc, s);
					pc.getInventory().removeItem(item, 1);
				} else {
					pc.sendPackets(new S_ServerMessage(74, item.getLogName()));
				}
				...
				startFishing(pc, itemId, locx, locy);
			} else if (use_type == 59 || use_type == 60 || use_type == 61 || use_type == 62 || use_type == 63
					|| use_type == 64 || use_type == 65 || use_type == 66 || use_type == 67 || use_type == 68
					|| use_type == 69 || use_type == 78) {
				// 回復/解毒/加速/勇敢/第三加速/魔眼/魔力回復/祝福/藍水/智慧/フローラ/課金ポーション (DB-driven: L1EtcItem add_*)
				// 【對齊原 JP 藥水邏輯】ディケイポーション中は飲めない（解毒除外）
				if (use_type != 60 && pc.hasSkillEffect(DECAY_POTION)) {
					pc.sendPackets(new S_ServerMessage(698));
					// \f1魔力によって何も飲むことができません。
					return;
				}
				...
				// 【藥水 buff 效果】applyEtcItemPotionEffect 只處理 add_* 即時加成；
				// 以下依 use_type 套用「需要計時/狀態」的 buff 效果
				if (use_type == 61) { // haste：加速藥水（綠水）
					L1BuffUtil.haste(pc, 1800 * 1000);
				} else if (use_type == 62) { // brave：勇敢藥水
					L1BuffUtil.brave(pc, 1800 * 1000);
				} else if (use_type == 63) { // third_speed：三段加速藥水
			...
			} else if (use_type == 70) { // 変身アイテム(poly): クライアント側HTML選択 → readS() → handleCommands
				// 【對齊 182】變身道具流程：客戶端打開 HTML 對話框 → 玩家選擇怪物 → 發送 polyName → 服務器處理
				if (s != null && !s.isEmpty()) {
					L1PolyMorph.handleCommands(pc, s);
					pc.getInventory().removeItem(item, 1);
				} else {
			...
			} else if (use_type == 71) { // HTML表示系 (DB-driven: 無 handler 時警告)
				_log.warning("C_UseItem: item_id=" + itemId + " (ShowMessage use_type=71) has no DB handler.");
				pc.sendPackets(new S_ServerMessage(74, item.getLogName()));
			} else if (use_type == 72) { // ルーレット (DB-driven: 無 handler 時警告)
				_log.warning("C_UseItem: item_id=" + itemId + " (Roulette use_type=72) has no DB handler.");
				pc.sendPackets(new S_ServerMessage(74, item.getLogName()));
			} else if (use_type == 73) { // テレポートアミュレット (DB-driven: 用 loc_x/loc_y/map_id 或警告)
				int locX = 0, locY = 0;
				short mapId = 0;
				if (item.getItem() instanceof L1EtcItem) {
					L1EtcItem etc = (L1EtcItem) item.getItem();
					locX = etc.getLocX();
					locY = etc.getLocY();
					mapId = etc.getMapId();
				}
				if (locX != 0 || locY != 0 || mapId != 0) {
					L1BuffUtil.cancelBarrier(pc);
					if (pc.getMap().isEscapable() || pc.isGm()) {
						L1Teleport.teleport(pc, locX, locY, mapId, pc.getHeading(), true);
						pc.getInventory().removeItem(item, 1);
					...
					_log.warning("C_UseItem: item_id=" + itemId + " (Teleport use_type=73) has no loc/map in DB.");
					pc.sendPackets(new S_ServerMessage(74, item.getLogName()));
				}
			} else if (use_type == 74) { // モンスター召喚 (DB-driven: 無 handler 時警告)
				_log.warning("C_UseItem: item_id=" + itemId + " (SpawnWand use_type=74) has no DB handler.");
				pc.sendPackets(new S_ServerMessage(74, item.getLogName()));
			} else if (use_type == 75) { // 家具 (DB-driven: 無 handler 時警告)
				_log.warning("C_UseItem: item_id=" + itemId + " (Furniture use_type=75) has no DB handler.");
				pc.sendPackets(new S_ServerMessage(74, item.getLogName()));
			} else if (use_type == 77) { // material (DB-driven: 無 handler 時警告)
				_log.warning("C_UseItem: item_id=" + itemId + " (Material use_type=77) has no DB handler.");
				pc.sendPackets(new S_ServerMessage(74, item.getLogName()));
			} else {
				int locX = 0, locY = 0;
				short mapId = 0;
				...
				if (locX != 0 || locY != 0 || mapId != 0) {
					...
					_log.warning("C_UseItem: item_id=" + item.getItemId() + " (etc teleport) has no loc/map in DB.");
					pc.sendPackets(new S_ServerMessage(74, item.getLogName()));
				}
			}
	...
	/** DB驅動：依 L1EtcItem 的 add_* 套用效果並消耗道具。無資料或非 EtcItem 時記錄警告、回傳 false（不崩潰）。 */
	private boolean applyEtcItemPotionEffect(L1PcInstance pc, L1ItemInstance item) {
		L1Item template = item.getItem();
		if (!(template instanceof L1EtcItem)) {
			_log.warning("C_UseItem: item_id=" + item.getItem().getItemId() + " is not L1EtcItem (missing DB handler).");
			return false;
		}
		L1EtcItem etc = (L1EtcItem) template;
		int addHp = etc.getAddHp();
		int addMp = etc.getAddMp();
		if (addHp > 0) {
			pc.setCurrentHp(Math.min(pc.getCurrentHp() + addHp, pc.getMaxHp()));
			pc.sendPackets(new S_HpUpdate(pc.getCurrentHp(), pc.getMaxHp()));
		}
		if (addMp > 0) {
			pc.setCurrentMp(Math.min(pc.getCurrentMp() + addMp, pc.getMaxMp()));
			pc.sendPackets(new S_MpUpdate(pc.getCurrentMp(), pc.getMaxMp()));
		}
		if (etc.getAddStr() != 0 || etc.getAddDex() != 0 || etc.getAddCon() != 0
				|| etc.getAddInt() != 0 || etc.getAddWis() != 0) {
		...
		if (etc.getAddSp() != 0 || etc.getAddMr() != 0) {
			pc.addSp(etc.getAddSp());
			pc.addMr(etc.getAddMr());
			pc.sendPackets(new S_SpMr(pc));
		}
		if (etc.getAddHpr() != 0 || etc.getAddMpr() != 0) {
			pc.addHpr(etc.getAddHpr());
			pc.addMpr(etc.getAddMpr());
		}
		if (etc.getAddHit() != 0 || etc.getAddDmg() != 0) {
			pc.addHitup(etc.getAddHit());
			pc.addDmgup(etc.getAddDmg());
		}
		if (etc.getAddBowHit() != 0 || etc.getAddBowDmg() != 0) {
	...
	private boolean useTeleportScroll(L1PcInstance pc, L1ItemInstance item, int objid) {
		...
		if (bookm != null) { // ブックマークを取得出来たらテレポート
			if (pc.getMap().isEscapable() || pc.isGm()) {
				...
				if (itemId == 203) { // マステレポートスクロール (182: 全体传送术的卷轴)
					for (L1PcInstance member : L1World.getInstance().getVisiblePlayer(pc)) {
						if (pc.getLocation().getTileLineDistance(
								member.getLocation()) <= 3
								&& member.getClanId() == pc.getClanId()
