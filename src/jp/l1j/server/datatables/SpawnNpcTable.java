...
public class SpawnNpcTable {
		...
		try {
			...
			while (rs.next()) {
				...
				if (l1npc == null) {
					...
					_spawntable.put(Integer.valueOf(l1spawn.getId()), l1spawn);
	...
	public void storeSpawn(L1PcInstance pc, L1Npc npc) {
		...
		try {
			...
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("INSERT INTO spawn_npcs SET npc_id=?,name=?,count=?,loc_x=?,loc_y=?,heading=?,map_id=?");
